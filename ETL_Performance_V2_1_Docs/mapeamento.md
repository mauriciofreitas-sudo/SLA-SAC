# ETL_Performance_V2 (1) — Mapeamento de Colunas

## Fluxo: ADO NET Source (vw_fPerformance) → ADO NET Destination (fPerformance)

**Idêntico ao ETL_Performance_V2.dtsx.** Todas as 43 colunas são passadas diretamente (passthrough).
...
## Atenção — Coluna NFDtEmissao vs. NFDTEmissao

| Contexto        | Nome usado        |
|-----------------|-------------------|
| Mapeamento SSIS | `NFDtEmissao`     |
| WHERE do DELETE | `NFDTEmissao`     |

Esta discrepância de case (`Dt` vs. `DT`) pode indicar um typo — SQL Server é case-insensitive por padrão para nomes de colunas, então funcionará, mas é inconsistente e pode causar confusão.
